...
void sx127x_init(sx127x_config_t *config, spi_inst_t *spi, unsigned int cs, unsigned int reset, unsigned long long frequency)
{
    config->spi = spi;
    config->cs = cs;
    config->reset = reset;
...
static uint8_t _sx127x_single_transfer(const sx127x_config_t *config, uint8_t address, uint8_t value)
{
    ...
    spi_write_blocking(config->spi, &address, 1);
    spi_write_read_blocking(config->spi, &value, &response, 1);
